...
function campo20 () {
    ...
    if ((elemento.length < 20 || elemento === "") || 
        (elemento2.length < 20 || elemento2 === "")) {
        alert("El nombre del dicos y el nombre del grupo debe tener 20 carácteres")
        result = false;
    }
    return result;
}
...
function validarAno () {
    ...
    if (isNaN(elemento) || elemento.value === "") {
        ...
        if (elemento.toString().length < 4) {
            alert("El año de publicación debe ser de 4 carácteres.");
            result = false; 
        }
    }
    return result;
...
function validarLocalizacion () {
    ...
    let result = true;
    if (isNaN(elemento)) {
        alert("La localización es erronea, debe ser un número de estanteria o vacío")
        result = false;
    }
    return result;
}